...
8. AgentRoom observes the Chatto reply through the connector's realtime listener or polling fallback.
...
- open the repo room
- find the AgentRoom approval request by visible text and decision ID
...
- observe room/thread activity through realtime or polling
...
- Chatto thread contains the AgentRoom approval request with the expected decision ID.
